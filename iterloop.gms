...
*  ---------------------------------------------------------------------------
*  Capital law of motion  (vdM eqs. 3.79-3.80; Ch.4 dynamics, item 3)
*  K0_t = K1_{t-1}: beginning-of-period stock = prior end-of-period stock.
*
*  Capital is a MOBILE factor here (cap in fm) with etaf(r,cap)=0, so xfteq
*  (eq. 3.73) pins xft(r,cap,t) = aft(r,cap,t): the shifter aft IS the
*  exogenous capital supply. We therefore drive aft, NOT xft.fx (fixing xft
*  would collide with the active xfteq(cap) and make the model infeasible).
*
*  Normalization: kstockeq gives kstock(t)=xft(cap,t)=aft(cap,t) in normalized
*  units (krat cancels the xft0(cap)/kstock0 coefficient exactly). The physical
*  identity K0_t = K1_{t-1} is  kstock(t)*kstock0 = kapEnd(t-1)*kapEnd0, hence
*  the kapEnd0/kstock0 factor.  (First dyn period: kapEnd.l(t0)=1 from cal.)
*  ---------------------------------------------------------------------------
   loop(cap,
      aft(r,cap,tsim)$kstock0(r) = kapEnd.l(r,tsim-1) * (kapEnd0(r)/kstock0(r)) ;
   ) ;
...
* fctts, fcttx, exptx and imptx are fixed in cal for all t in lines 169, 171, 354, and 371 . prdtx is set in cal for all t in line 321
if(MRIO,
...
*  GTAP-LU
pvaNLND.lo(r,a,tsim)$vaNLNDFlag(r,a)    = 0.001*pvaNLND.l(r,a,tsim-1) ;
*pvaLNDNDX.lo(r,a,tsim)$vaLNDFlag(r,a)   = max(1e-6, 0.001*pvaLNDNDX.l(r,a,tsim-1)) ;
pvaLNDNDX.lo(r,a,tsim)$vaLNDFlag(r,a)   = 0.001*pvaLNDNDX.l(r,a,tsim-1) ;
pvaLND.lo(r,a,tsim)$vaLNDFlag(r,a)      = 0.001*pvaLND.l(r,a,tsim-1) ;
...
pfy.lo(r,fp,a,tsim)$xfFlag(r,fp,a)      = 0.001*pfy.l(r,fp,a,tsim-1) ;
*pfa.lo(r,fp,a,tsim)$xfFlag(r,fp,a)      = max(1e-6, 0.001*pfa.l(r,fp,a,tsim-1)) ;
pfa.lo(r,fp,a,tsim)$xfFlag(r,fp,a)      = 0.001*pfa.l(r,fp,a,tsim-1) ;
*pft.lo(r,fm,tsim)$xftFlag(r,fm)         = 0.001*pft.l(r,fm,tsim-1) ;
pftNDX.lo(r,fm,tsim)$xftFlag(r,fm)      = 0.001*pftNDX.l(r,fm,tsim-1) ;
...
* Household consumption quantity safeguard
*xa.lo(r,i,h,tsim)$xaFlag(r,i,h)
*   = 0.001*xa.l(r,i,h,tsim-1) ;
...
*  GTAP-LU
   vaNLND.fx(r,a,tsim)$(not vaNLNDFlag(r,a)) = 0 ;
   vaLND.fx(r,a,tsim)$(not vaLNDFlag(r,a))   = 0 ;
   px.fx(r,a,tsim)$(not xpFlag(r,a))       = px.l(r,a,t0) ;

   vtfp.fx(r,a,t)$(not vaFlag(r,a))         = 1 ;
   nlndtfp.fx(r,a,t)$(not vaFlag(r,a))      = 1 ;
   lndtfp.fx(r,a,t)$(not vaFlag(r,a))       = 1 ;
...
*  GTAP-LU
   pvaNLND.fx(r,a,tsim)$(not vaNLNDFlag(r,a))  = pvaNLND.l(r,a,t0) ;
   pvaLND.fx(r,a,tsim)$(not vaLNDFlag(r,a))    = pvaLND.l(r,a,t0) ;
   pvaLNDNDX.fx(r,a,tsim)$(not vaLNDFlag(r,a)) = pvaLNDNDX.l(r,a,t0) ;
   ...
   pftNDX.fx(r,fm,tsim)$(not xftFlag(r,fm)) = pftNDX.l(r,fm,t0) ;
...
*  Fix lags. (This is only for variables that require lags during runtime in model.gms). The intertemporal updating of all endog variables happens in mvar.gms
...
if(years(tsim) ne firstYear,
   ...
   vtfp.fx(r,a,tsim-1)       = vtfp.l(r,a,tsim-1) ;
   nlndtfp.fx(r,a,tsim-1)    = nlndtfp.l(r,a,tsim-1) ;
   lndtfp.fx(r,a,tsim-1)     = lndtfp.l(r,a,tsim-1) ;
...
if (years(tsim) ne firstYear,
    display axp.l, lambdand.l, lambdava.l, lambdaio.l, vtfp.l, nlndtfp.l, lndtfp.l, lambdaf.l, pf.l, xf.l, pa.l,
    xa.l, pe.l, pefob.l, pmcif.l, pm.l, xw.l, ptmg.l, psave.l, pi.l, uh.l, yc.l, pabs.l, pmuv.l, pfact.l, pwfact.l
    gdpmp.l, rgdpmp.l, pgdpmp.l;
);
